...
# else
void segment_graph_region(universe* mess, vector<edge>* edges_remain, 
		vector<edge>* edges_region, float c_reg, int level, int max_level) {
# endif
	double w_at_i = 0, en_a = 0, en_b = 0;
	...
	for (int i = 0; i < ((int) edges_region->size()); i++) {
		...
		if (a != b) {
			w_at_i = edges_region->at(i).w;
			en_a = mess->get_mst(a) + (c_reg / mess->get_size(a));
			en_b = mess->get_mst(b) + (c_reg / mess->get_size(b));
