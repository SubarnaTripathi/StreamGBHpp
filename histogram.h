...
public:
	...
	inline void addOriSample(TYPE s, TYPE v)
	{
# if 0
		double val = computeOriVal(v);
		addWeightedSample(s,val);
# else
		addOriWeightedSample(s,v,1);
# endif
	}
	
	double computeOriVal(TYPE v)
	{
		double val;		
		// compute the val; round
		val = v; //(int)(v+0.5);
			
		return val;
	}

	void addOriWeightedSample(TYPE s, TYPE v, double w)
	{
		int bin=-1;
		int val =-1;
	...
	double l1distanceOri(Histogram& H)
	{
		double d = 0.0;
	...
	void mergeOriHistogram(Histogram& H)
	{
		for (int i=0;i<numBins;i++)
		{
			bins[i] = (bins[i] + H.bins[i]);
		}
		mass += H.getMass();
	}
